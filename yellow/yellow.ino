...
/* ============ 完全独立的 6 份代码 ============ */
#define PWM1 3  // rightforward
#define INA1 4
#define INB1 5

#define PWM2 6  // leftforward
#define INA2 7
#define INB2 8
...
void superbackward() {
  ...
  controlMotors(255, 0, 255, 0, 255, 0, 255, 1);
}

void superforward() {
  delay(50);
  // controlMotors(255, true, 170, true, 0, true, 0, false);
  controlMotors(170, 1, 255, true, 200, 1, 255, 0);
}

void backward() {
  delay(50);
  controlMotors(75,0, 75, false, 62, 0, 75, 1);
  // controlMotors(30, false, 30, false, 30, false, 30, true);
}

void forward() {
  delay(50);
  controlMotors(75, 1, 75, true, 62, 1, 75, 0);
...
void left() {
  ...
  controlMotors(85, 0, 127, true, 100, 0, 127, 0);
}

void right() {
  delay(50);
  // controlMotors(255, true, 255, false, 220, true, 0, true);
  controlMotors(120, 1, 127, 0, 100, 1, 127, 1);
}
...
void rotateright(int angle) {
  // 计算需要的延时时间（每度约2.8毫秒）
  // int delayTime = angle * 2.8;
  int delayTime = angle * 4.0;
...
//此函数检测是否重新登上擂台，若检测到大角度波动则进入autofight正常循台模式
void check_mpu_up(float limit){
  if((fabs(pitch) > limit || fabs(roll) > limit)&&mode=="drop_rush"){
    Serial.printf("检测到倾斜: Pitch=%.1f, Roll=%.1f\n", pitch, roll);

    backward();
    delay(400);
    mode="autofight";
    show_oled("autofight");
    Serial.println("✅ 检测到上台倾斜，切换回autofight");
    dropRunning = false;  // 重置标志
  }
}
//当小车掉下擂台（drop_check）模式时，通过红外检测来调整重新上台的方向，调整完进入drop_rush模式
void drop_check(){
  // 读取红外传感器状态
    int fl = getio_FL();
    int fr = getio_FR();
    ...
    // 当前面两个、后面两个、左=1、右=0 或右=1、左=0 或左右都=0 时停止
    if (((fl && fr && bl && br) && ((l && !r) || (!l && r) || (!l && !r)))) {
      stop();
      Serial.println("🛑 检测到边界组合 -> 停止");
      mode="drop_rush";
      show_oled("drop_rush");
      return;
    }

    // 当前面两个=1、左=0、右=1 -> 左转135°
    else if ((fl && fr) && (!l && r)) {
      Serial.println("↩️ 左转135°");
      rotateLeft(135);
      delay(200);
      stop(); // 最终确保停止
      mode="drop_rush";
    ...
    // 当前面两个=1、后两个=0、左=1、右=0 -> 右转135°
    else if ((fl && fr) && (!bl && !br) && (l && !r)) {
      Serial.println("↪️ 右转135°");
      rotateright(135);
      delay(200);
      stop(); // 最终确保停止
      mode="drop_rush";
    ...
    // 当前面两个=0、后两个=1、左=1、右=0 -> 右转45°
    else if ((!fl && !fr) && (bl && br) && (l && !r)) {
      Serial.println("↪️ 右转45°");
      rotateright(45);
      delay(200);
      stop(); // 最终确保停止
      mode="drop_rush";
    ...
    // 当前面两个=0、后两个=1、左=0、右=1 -> 左转45°
    else if ((!fl && !fr) && (bl && br) && (!l && r)) {
      Serial.println("↩️ 左转45°");
      rotateLeft(45);
      delay(200);
      stop(); // 最终确保停止
      mode="drop_rush";
...
//当调整完方向时（drop_rush模式），进行冲台，并且一直检测是否冲台成功，若成功则进入autofight正常循台模式，若4s内不成功，则进入drop_check模式，继续调整方向
void drop_rush(){
    static unsigned long rushStartTime = 0;
    
    if (rushStartTime == 0) {
        rushStartTime = millis();
        Serial.println("🚀 开始drop_rush冲台");
    }
    
    controlMotors(225, false, 225, false, 225, false, 225, true);
    
    // 冲台1秒后开始检测上台
    if (millis() - rushStartTime > 1000 && upConfirmTime == 0) {
        if((fabs(pitch) > 25 || fabs(roll) > 25)){
            Serial.println("📈 检测到上台倾斜，进入确认期");
            upConfirmTime = millis();
        }
    }
    ...
    if(upConfirmTime > 0){
        // 确认期结束，正式上台
        if(millis() - upConfirmTime > CONFIRM_DURATION){
            Serial.println("✅ 确认期结束，上台成功");
            backward();
            delay(400);
            mode = "autofight";
            show_oled("autofight");
        ...
        // 确认期内继续冲台
        controlMotors(225, 1, 225, false, 225, false, 225, true);
    }
    
    // 总时间超时
    if (millis() - rushStartTime > 5000) { // 总时间延长到5秒
        mode = "drop_check";
        show_oled("drop_check");
        rushStartTime = 0;
        upConfirmTime = 0;
        dropRunning = false;
        Serial.println("⏰ drop_rush超时，返回drop_check");
    }
}
